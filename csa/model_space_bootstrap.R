...
for (csasel in csas){
  ...
  for (i in 1:80){
    print(i)
    mod <- feols(vader ~ wbgt + raining + srad | dow + doy + tod + fips + year + month, 
                 data=sel[sample(1:nrow(sel), nrow(sel), replace=T), ])
    base_res <- tidy(mod) %>%
      filter(term =='wbgt')
    all_base <- c(base_res$estimate, all_base)
  }
  base_res <- data.frame(min = quantile(all_base, probs=0.025),
                         max = quantile(all_base, probs=0.975),
                         med = quantile(all_base, probs=0.5)) %>%
    mutate(term = 'wbgt',
           id = csasel)
  ...
  for (i in 1:80){
    print(i)
    mod <- feols(vader ~ wbgt*income_percap + raining*income_percap + srad*income_percap | dow + doy + tod + fips + year + month, 
                 data=sel[sample(1:nrow(sel), nrow(sel), replace=T), ])
    inc_res <- tidy(mod) %>%
      filter(term =='wbgt:income_percap') %>%
      select(estimate)
    all_inc <- c(inc_res, all_inc)
  }
  inc_res <- data.frame(min = quantile(all_inc, probs=0.025),
                        max = quantile(all_inc, probs=0.975),
                        med = quantile(all_inc, probs=0.5)) %>%
    mutate(term = 'wbgt:income_percap',
           id = csasel)
  ...
  for (i in 1:80){
    print(i)
    mod <- feols(vader ~ wbgt*race_q + raining*race_q + srad*race_q | dow + doy + tod + fips + year + month, 
                 data=sel[sample(1:nrow(sel), nrow(sel), replace=T), ])
    race_res <- tidy(mod) %>%
      filter(term =='wbgt:race_q') %>%
      select(estimate)
    all_race <- c(race_res, all_race)
  }
  race_res <- data.frame(min = quantile(all_race, probs=0.025),
                         max = quantile(all_race, probs=0.975),
                         med = quantile(all_race, probs=0.5)) %>%
    mutate(term == 'wbgt',
           id = csasel)

  allres <- bind_rows(allres, base_res, inc_res, race_res)
}
