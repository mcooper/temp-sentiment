...
data <- fread('all.csv')
data$impervious_q <- cut2(data$impervious, g=2)
qs <- levels(data$impervious_q)
...
formula <- paste0("vader ~ ", 
                     piece.formula("temp", knots[['temp']], "impervious_q", 'factor'), ' + ',
                     piece.formula("precip", knots[['precip']], "impervious_q", 'factor'), ' + ',
                     piece.formula("srad", knots[['srad']], "impervious_q", 'factor'),
...
preddf <- make_groups(preddf, 'impervious_q', qs)
...
preddf$impervious_q <- factor(preddf$impervious_q)

curve <- ggplot(preddf) + 
  geom_line(aes(x=temp, y=contrast, color=impervious_q)) + 
  geom_ribbon(aes(x=temp, ymin=ymin, ymax=ymax, fill=impervious_q), alpha=0.5) + 
...
ggsave('~/temp-sentiment/res/temp-impervious_q.png', width=6, height=7)
...
curve <- ggplot(preddf) + 
  geom_line(aes(x=log(precip + 1), y=contrast, color=impervious_q)) + 
  geom_ribbon(aes(x=log(precip + 1), ymin=ymin, ymax=ymax, fill=impervious_q), alpha=0.5) + 
...
plot_grid(curve, hist, align='v', axis='rl', ncol=1, rel_heights=c(0.8, 0.2))
ggsave('~/temp-sentiment/res/precip-impervious_q.png', width=6, height=7)
...
curve <- ggplot(preddf) + 
  geom_line(aes(x=srad, y=contrast, color=impervious_q)) + 
  geom_ribbon(aes(x=srad, ymin=ymin, ymax=ymax, fill=impervious_q), alpha=0.5) + 
  labs(x='', y='Predicted Mood of Tweet',
       fill = 'Census Block\nIncome Per-Capita\n(Percentile)',
       color = 'Census Block\nIncome Per-Capita\n(Percentile)' ) +
...
plot_grid(curve, hist, align='v', axis='rl', ncol=1, rel_heights=c(0.8, 0.2))
ggsave('~/temp-sentiment/res/srad-impervious_q.png', width=6, height=7)
