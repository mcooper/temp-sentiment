...
data <- fread('all.csv')
data$tree_q <- cut2(data$tree, g=2)
qs <- levels(data$tree_q)
...
formula <- paste0("vader ~ ", 
                     piece.formula("temp", knots[['temp']], "tree_q", 'factor'), ' + ',
                     piece.formula("precip", knots[['precip']], "tree_q", 'factor'), ' + ',
                     piece.formula("srad", knots[['srad']], "tree_q", 'factor'),
...
preddf <- make_groups(preddf, 'tree_q', qs)
preddf$tree_q <- factor(preddf$tree_q)
preddf$tree_q <- relevel(preddf$tree_q, ref='0')
...
curve <- ggplot(preddf) + 
  geom_line(aes(x=temp, y=contrast, color=tree_q)) + 
  geom_ribbon(aes(x=temp, ymin=ymin, ymax=ymax, fill=tree_q), alpha=0.5) + 
...
ggsave('~/temp-sentiment/res/temp-tree_q.png', width=6, height=7)
...
curve <- ggplot(preddf) + 
  geom_line(aes(x=log(precip + 1), y=contrast, color=tree_q)) + 
  geom_ribbon(aes(x=log(precip + 1), ymin=ymin, ymax=ymax, fill=tree_q), alpha=0.5) + 
...
plot_grid(curve, hist, align='v', axis='rl', ncol=1, rel_heights=c(0.8, 0.2))
ggsave('~/temp-sentiment/res/precip-tree_q.png', width=6, height=7)
...
curve <- ggplot(preddf) + 
  geom_line(aes(x=srad, y=contrast, color=tree_q)) + 
  geom_ribbon(aes(x=srad, ymin=ymin, ymax=ymax, fill=tree_q), alpha=0.5) + 
  labs(x='', y='Predicted Mood of Tweet',
       fill = 'Census Block\nTree Cover Percent\n(Percentile)',
       color = 'Census Block\nTree Cover Percent\n(Percentile)' ) +
...
plot_grid(curve, hist, align='v', axis='rl', ncol=1, rel_heights=c(0.8, 0.2))
ggsave('~/temp-sentiment/res/srad-tree_q.png', width=6, height=7)
