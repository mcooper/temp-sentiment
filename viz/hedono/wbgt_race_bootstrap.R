...
preddf <- preddf %>%
  gather(key, value, -wbgt, -race_white, -hedono) %>%
...
curve <- ggplot(preddf) + 
  geom_line(aes(x=wbgt, y=pred, color=race_white)) + 
...
ggsave('~/temp-sentiment/res/hedono-wbgt-race.png', width=4.5, height=4)
